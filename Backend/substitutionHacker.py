...
def substitutionHacker(cipherText):
    ...
    # Create the key based on the intersected mapping
    for cipherLetter in LETTERS:
        if len(intersectedMapping[cipherLetter]) == 1:
            key[cipherLetter] = intersectedMapping[cipherLetter][0]
        else:
            key[cipherLetter] = '_'
...
def main():
    substitutionHacker("Oybb brkl vsu br usyev iru bvs tbyut, cpb Gsurqwey bvrpjvb wb fyt bvs ortb uwlwepkrpt ylgwes tvs'l sgsu useswgsl. Tpus, wb vyl cssq fskk-osyqwqj fvsq vs tywl wb, cpb tvs lwlq'b pqlsutbyql fvm yqmrqs frpkl fyqb br tpjjstb trosbvwqj bvyb frpkl kwbsuykkm hwkk mrp wi mrp yebpykkm oyqyjsl br yevwsgs wb. Bvsus't qr fym vs'l yjuss.")
